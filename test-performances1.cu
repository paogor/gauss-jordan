...
//   for(int j = 0; j<n; ++j)
  ...
  host_vector<double> h_A0, h_A1, h_A2, h_A3, h_A4;
...

  {
    device_vector<double> d_A0 = A;
    {
      ...
      gauss_jordan0<<<m,n,n*n*sizeof(double)>>>(n, raw_pointer_cast(d_A0.data()));
    }
    h_A0 = d_A0;
  }
...

  {
    device_vector<double> d_A2 = A;
    {
      ...
      gauss_jordan2<<<m,n,(n*n+1)*sizeof(double)>>>(n, raw_pointer_cast(d_A2.data()));
    }
    h_A2 = d_A2;
  }
...

  {
    ...
    {
      CUDATIMER t(times_ms + 5);
      gauss_jordan4<<<dim3(m,1,1),dim3(n,n,1),(n*n+1)*sizeof(double)>>>(n, raw_pointer_cast(d_A4.data()));
    }
    h_A4 = d_A4;
  }
  ...
  std::cout<<n<<", "<<m;
  for(size_t  i = 0; i<6; ++i) std::cout<<", "<<times_ms[i];
  std::cout<<";"<<std::endl;
